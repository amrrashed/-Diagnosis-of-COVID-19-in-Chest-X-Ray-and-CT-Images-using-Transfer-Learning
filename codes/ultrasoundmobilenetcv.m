...
%%cross validation
%load images
digitDatasetPath = fullfile('D:\covid project\ADATASETS\eng_maidataset_modified');
...
%     augtestimds = augmentedImageDatastore([224 224],imdsTest);
    ...
    % Save the Independent MOBILENet Architectures obtained for each Fold
    save(sprintf('mobilenetv2_%d_among_%d_folds',fold_idx,num_folds),'netTransfer','test_idx','train_idx','labeltest','labeltrain');
    delete(findall(0))
