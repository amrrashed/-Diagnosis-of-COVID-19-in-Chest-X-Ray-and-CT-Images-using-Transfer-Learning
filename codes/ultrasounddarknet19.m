...
%%cross validation
%load images
digitDatasetPath = fullfile('G:\new researches\mansour paper\dataset224');
...
%           'LearnRateSchedule','piecewise', ...
    ...
    auimds = augmentedImageDatastore([256 256],imdsTrain,'DataAugmentation',augmenter);
    
    % Training
    netTransfer = trainNetwork(auimds,layers,options);
%    netTransfer = trainNetwork(imdsTrain,layers,options);
    % Resizing all testing images to [224 224] for ResNet architecture   
     augtestimds = augmentedImageDatastore([256 256],imdsTest);
  ...
  %  [predicted_labels(test_idx),posterior(test_idx,:)] = classify(netTransfer,imdsTest,'ExecutionEnvironment','GPU');
    % Save the Independent ResNet Architectures obtained for each Fold
    save(sprintf('darknet19_%d_among_%d_folds',fold_idx,num_folds),'netTransfer','test_idx','train_idx');
    % delete(findall(0))
...
title('Confusion Matrix: darknet19 ');
%ROC CURVE
