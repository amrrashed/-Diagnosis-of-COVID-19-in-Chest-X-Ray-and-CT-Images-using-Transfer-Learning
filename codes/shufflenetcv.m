function [t,x] =shufflenetcv(path,optimizer,augmentation,numfold)
...
lgraph = replaceLayer(lgraph,'node_202',newFCLayer);
newClassLayer = softmaxLayer('Name','new_softmax');
lgraph = replaceLayer(lgraph,'node_203',newClassLayer);
newClassLayer = classificationLayer('Name','new_classoutput');
lgraph = replaceLayer(lgraph,'ClassificationLayer_node_203',newClassLayer);
    
    % Preprocessing Technique
   
    % Training Options, we choose a small mini-batch size due to limited images 
    options = trainingOptions(optimizer,...
        'MaxEpochs',10,'MiniBatchSize',30,...
        'Shuffle','every-epoch', ...
...
%     % Data Augumentation
      augmenter = imageDataAugmenter( ...
         ...
         'RandYReflection',0,'RandXShear',[-0.05 0.05],'RandYShear',[-0.05 0.05]);
